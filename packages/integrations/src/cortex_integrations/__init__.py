...
def resolve_connector_mode(credential: Optional[str] = None, explicit_mock: Optional[bool] = None) -> ConnectorMode:
    ...
    if explicit_mock is True:
        if is_prod:
            raise RuntimeError("Explicit MOCK mode is strictly forbidden in production environments.")
        return ConnectorMode.MOCK

    if credential and credential not in ("placeholder", "mock", "test_key", ""):
        return ConnectorMode.LIVE

    if is_prod:
        # In production, missing credentials means DISABLED, NEVER silent MOCK!
...
class EmailToolExecutor:
    ...
    async def execute(self, params: Dict[str, Any], execution_context: Optional[Any] = None) -> Dict[str, Any]:
        ...
        if self.mode == ConnectorMode.MOCK or not HAVE_SENDGRID:
            ...
            msg_id = f"msg_mock_{now_utc.strftime('%Y%m%d%H%M%S')}_{abs(hash(to_email)) % 10000}"
            return {
                "delivered": True,
                "recipient": to_email,
                "subject": subject,
        ...
        try:
            ...
            msg_id = response.headers.get("X-Message-Id", "sg_live")
            return {
...
class SMSToolExecutor:
    """Production Twilio SMS dispatcher with explicit mode handling."""
...
class CRMToolExecutor:
    ...
    async def execute(self, params: Dict[str, Any], execution_context: Optional[Any] = None) -> Dict[str, Any]:
        ...
        if self.mode == ConnectorMode.MOCK or not HAVE_HUBSPOT:
            ...
            rec_id = f"hubspot_mock_{abs(hash(lead_id)) % 100000}"
            return {
        ...
        if self.mode == ConnectorMode.MOCK or not HAVE_STRIPE:
            ...
            if action == "create_payment_link":
                ...
                return {
                    ...
                    "external_id": payload.get("payment_intent_id", mock_id),
                    "provider": "stripe",
                    "amount": payload.get("amount", 0),
                    "currency": payload.get("currency", "usd"),
        ...
        if self.mode == ConnectorMode.MOCK:
            ...
            if action == "create_ticket":
                ...
                return {
                    ...
                    "external_id": str(payload.get("ticket_id", mock_id)),
                    "provider": "zendesk",
        ...
        if self.mode == ConnectorMode.MOCK:
            ...
            if action == "check_availability":
                ...
                return {
                    ...
                    "external_id": payload.get("booking_id", "cal_book_001"),
                    "provider": "calendly",
